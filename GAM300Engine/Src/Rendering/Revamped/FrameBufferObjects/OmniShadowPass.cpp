...
namespace TDS
{
	...
	OmniShadowPass::OmniShadowPass(Vec2 DimensionInput)
	{
		m_Resizeable = true;
		m_AutoSync = true;
	...
	void OmniShadowPass::PrepareRenderPass(VkRenderPass& renderpass)
	{
		GraphicsManager& mgr = GraphicsManager::getInstance();
	...
	void OmniShadowPass::PrepareCubeMapTexture()
	{
	...
	bool OmniShadowPass::Resize(std::uint32_t width, std::uint32_t height)
	{
		Destroy();
		PrepareRenderPass(m_Renderpass);
	...
	void OmniShadowPass::InitResizeFunctionPointer()
	{
		if (m_Resizeable)
		{
