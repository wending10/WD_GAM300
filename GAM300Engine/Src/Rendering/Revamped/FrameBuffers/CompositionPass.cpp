...
namespace TDS
{
	CompositionPass::CompositionPass(Vec2 DimensionInput)
	{
		m_Resizeable = true;
		m_AutoSync = true;
	...
	bool CompositionPass::Resize(std::uint32_t width, std::uint32_t height)
	{
	...
	void CompositionPass::GetBlendAttachments(BlendContainer& blendCont, PipelineCreateEntry& createEntry)
	{
		blendCont.resize(1);
